...
## 6. (Опционально) Наполнить демо-данными

Чтобы магазин и запись сразу были не пустыми (удобно показывать клиентам):

```bash
python -m cloud5.cli seed --tenant-id 1
```

Добавит демо-категории, товары, услуги и базу знаний. Повторный запуск
ничего не дублирует.
